...
#dat$hit <-grepl("3029|3030",dat$`MO.Codes`,perl=TRUE)
dat$hit <-grepl("3029|3030",dat$`MO.Codes`,perl=TRUE)
...
require(neuralnet)
require(nnet)
nn <- neuralnet(hit ~ Time.ccurred.rescaled+Victim.Age_A+Victim.Age_B+ Victim.Age_C+Victim.Age_D+
                  Victim.Sex_F+Victim.Sex_M+Victim.Sex_X+Victim.Descent_B+Victim.Descent_H+Victim.Descent_O+
                  Victim.Descent_Other_Descent+Victim.Descent_W+Council.Districts_1+Council.Districts_2+
                  Council.Districts_3+Council.Districts_4+Council.Districts_5+Council.Districts_6+
                  Council.Districts_7+Council.Districts_8+Council.Districts_9+Council.Districts_10+
                  Council.Districts_11+Council.Districts_12+Council.Districts_13+Council.Districts_14+
                  Council.Districts_15,data = dat, linear.output = F, hidden = 3)

require(caret)
predict <- compute(nn, data.frame(df$Salt, df$Fat))
predicted.class=apply(predict$net.result,1,which.max)-1
confusionMatrix(ifelse(hit=="1", "non", "hit and run"), dat$hit)
